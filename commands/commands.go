...
func commandListEmbed() *discordgo.MessageEmbed {
	...
	embed := &discordgo.MessageEmbed{
		Type:        "rich",
		Color:       52,
		Description: "List of all commands currently available in version " + Version + " of FactoCord",
...
func RunCommand(input string, s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	//Suprnova74: remmed out, testing code from different fork
	s.ChannelMessageSend(support.Config.FactorioChannelID, "Command not found or the command is missing required arguments!")
	s.ChannelMessageSend(m.ChannelID, "Command not found or the command is missing required arguments!")
}
